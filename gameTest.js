document.addEventListener("DOMContentLoaded", () => {
  ...
  function submitPlayerName() {
    ...
    if (!sanitized) {
      if (nameErrorLabel) {
        nameErrorLabel.textContent = "Bitte einen vernÃÂ¼nftigen Namen (2-16 Zeichen, keine Beleidigungen) eingeben.";
      }
      return;
    }
  ...
  function drawEffectCircles() {
    const effects = [
      ...
      { key: "bigTimer", dur: DURATIONS.big, color: "#ff8899", label: "GroÃÂ" },
...
const assets = {
  ...
  // Trail
  const trail = [];
  const trailMaxLength = 40;
  let trailSampleTimer = 0;
  const trailSampleInterval = 0.018;
  ...
  const SCORE_TAUNTS = [
    ...
    "Der Boss schaut schon nervoÃÂ¨s.",
    "Break- Nicht heute.",
...
Dieser Text wird später ersetzt.
Schickt mir den finalen Story-Text für Boss 2.`,
    3: `Boss-Intro: Placeholder

Dieser Text wird später ersetzt.
Schickt mir den finalen Story-Text für Boss 3.`,
    4: `Boss-Intro: Placeholder

Dieser Text wird später ersetzt.
Schickt mir den finalen Story-Text für Boss 4.`,
    5: `Boss-Intro: Placeholder

Dieser Text wird später ersetzt.
Schickt mir den finalen Story-Text für Boss 5.`,
    6: `Boss-Intro: Placeholder

Dieser Text wird später ersetzt.
Schickt mir den finalen Story-Text für Boss 6.`,
  };
  ...
  let bossStage = 0; // progressive VerstÃÂ¤rkung nach jedem Boss
  ...
  function flap() {
    ...
    // Spin alle 25 SprÃÂ¼nge sicher auslÃÂ¶sen (random LÃÂ¤nge 3?5s)
    if (totalFlaps > 0 && totalFlaps % 25 === 0) {
      player.spinTimer = Math.max(player.spinTimer, 3 + Math.random() * 2);
    }
    // ZusÃÂ¤tzlich zufÃÂ¤lliger Spin alle 50 Klicks (LÃÂ¤nge 2?4s)
  ...
  function createBoss(id) {
    ...
    return {
      ...
      width: 300,
      height: 300,
      hp: Math.round(220 * (1 + bossStage * 0.15)),
      maxHp: Math.round(220 * (1 + bossStage * 0.15)),
      shotTimer: 0,
      shotInterval: (0.55 / (1 + bossStage * 0.12)) * 2.5,
  ...
  function defeatBoss(id) {
    ...
    if (id === 6) {
      gameWon = true;
      gameRunning = true;
      gameOver = false; // Weiterfliegen fÃÂ¼r Highscore
    }
  }
  ...
  function updateBoss(dt) {
    ...
    if (boss.shotTimer >= boss.shotInterval) {
      ...
      if (boss.id === 4) {
        ...
        if (boss.id === 3) {
          ...
          // abgeschwaechter Maschinengewehr-Modus
        ...
        if (bossObstacles.length < maxObs) {
          ...
          bossObstacles.push({
            ...
            gapY: gy,
            gap: gap,
            speed: boss.id === 3 ? 150 : 130,
            vy: boss.id === 3 ? (Math.random() < 0.5 ? 30 : -30) : 0,
          });
        }
        ...
        for (let k = 0; k < 6; k++) {
          ...
          shootBossProjectile(boss, a, 420);
        }
        for (let k = 0; k < 6; k++) {
          const a = (Math.PI * 2 * k) / 6 + boss.t * 0.5;
          shootBossProjectile(boss, a, 340);
        }
    ...
    // Beam damage (deutlich abgeschwÃÂ¤cht, Ticks alle 0.6s)
  ...
  function update(dt) {
    ...
    if (bossTransitionActive) {
      ...
      // wÃÂ¤hrend der Transition keine Welt-Updates
      return;
    }
    ...
    // Keine Gravitation wÃÂ¤hrend Boss-Vorbereitung
    if (!pendingBossId) {
...
function drawUI() {
  ...
  if (showIntro) {
    ...
    const effectCards = [
      { title: "GEIST", desc: "durch SÃÂ¤ulen gleiten", color: "#92f4ff" },
      { title: "SCHILD", desc: "blockt Treffer", color: "#8fb6ff" },
      { title: "DOPPEL", desc: "+2 Punkte pro SÃÂ¤ule", color: "#ffe066" },
      { title: "ZEITLUPE", desc: "SÃÂ¤ulen halb so schnell", color: "#9cff9c" },
